...
CREATE POLICY "orders_users_own" ON orders
    FOR ALL 
    USING (auth.uid() = user_id)
    WITH CHECK (auth.uid() = user_id);

-- Create simple, working RLS policies for order_items
CREATE POLICY "order_items_users_own" ON order_items
    FOR ALL 
    USING (
        EXISTS (
...
CREATE POLICY "inventory_logs_authenticated" ON inventory_logs
    FOR ALL 
